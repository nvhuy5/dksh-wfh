...
def task_execute(
    ...
    try:
        ...
        ctx.run(lambda: handle_task(file_path, celery_id, rerun_attempt))
        # ===
...
def _resolve_step_api(step, file_processor, context, step_config):    
    ...
    if not config_metadata:
        ...
        if "runner" in config_metadata:
            ...
            config_api = config_metadata["runner"](config_api_ctx)
        else:
            ...
            if method == "get":
                config_api = config_api_connector.get()
            elif method == "post":
                config_api = config_api_connector.post()
